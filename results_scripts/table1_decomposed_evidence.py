#!/usr/bin/env python3
"""
Table 1: Decomposed Evidence for Benchmark Complexes

Purpose
-------
Computes the four interpretable evidence channels for benchmark protein complexes, including antibody-antigen and enzyme-inhibitor examples used in the decomposed-evidence analysis.

Expected outputs
----------------
Writes antibody_antigen_decomposed_evidence.csv and enzyme_inhibitor_decomposed_evidence.csv.
...
# ============================================================================
# CELL E1: Generic RCSB entity selector for a different interaction type
# Example below is for ENZYME-INHIBITOR
# ============================================================================
...
PDB_IDS = [
    ...
    "2PTC",   # trypsin-pancreatic trypsin inhibitor type complex
    "1TAB",
    "1BXI",
    "1CGI",
    "1AVX",
...
LEFT_KEYWORDS  = [   # "enzyme" side
    "enzyme", "protease", "trypsin", "barnase", "ribonuclease",
    "kinase", "caspase", "peptidase", "hydrolase", "catalytic"
]

RIGHT_KEYWORDS = [   # "inhibitor" side
    "inhibitor", "barstar", "bpti", "trypsin inhibitor",
    "inhibitory protein", "serine protease inhibitor"
]
...
def keyword_hit(desc: str, keys) -> bool:
    return any(k in desc for k in keys)

def fetch_pair_by_keywords(pdb_id: str, left_keywords, right_keywords, bad_keywords=None):
    ...
    for eid in entity_ids:
        ...
        if len(seq) < 20:
            continue
        if any(k in desc for k in bad_keywords):
            continue
        ...
        ents.append({
            ...
            "chains": chains,
            "len": len(seq),
            "is_left": keyword_hit(desc, left_keywords),
            "is_right": keyword_hit(desc, right_keywords),
        })

    if len(ents) == 0:
        return None

    lefts  = [e for e in ents if e["is_left"]]
    rights = [e for e in ents if e["is_right"]]

    if len(lefts) == 0 or len(rights) == 0:
        return None

    left  = sorted(lefts,  key=lambda e: (-e["len"], e["entity_id"]))[0]
    right = sorted(rights, key=lambda e: ( e["len"], e["entity_id"]))[0]
    ...
    return {
        ...
        "left_desc": left["desc"],
        "left_chain": ",".join(left["chains"]),
        "left_seq": left["seq"],
        "right_entity": right["entity_id"],
        "right_desc": right["desc"],
        "right_chain": ",".join(right["chains"]),
        "right_seq": right["seq"],
    }
...
for pdb_id in PDB_IDS:
    try:
        ...
        if pair is None:
            print(f"[skip] {pdb_id}: no enzyme-inhibitor split found")
        else:
            print(f"[ok]   {pdb_id}: LEFT={pair['left_chain']} | RIGHT={pair['right_chain']}")
            pairs.append(pair)
    except Exception as e:
        print(f"[skip] {pdb_id}: {e}")

etype_pairs = pd.DataFrame(pairs)
print("\nRecovered pairs:", len(etype_pairs))
etype_pairs[["pdb", "left_chain", "right_chain", "left_desc", "right_desc"]]


# ==============================================================================

# ============================================================================
# CELL E2: Compute deterministic 4 scores + model prediction
# ============================================================================

if len(etype_pairs) == 0:
    raise ValueError("Recovered pairs = 0. Try different PDB IDs or keyword lists.")

rows = []
for _, r in etype_pairs.iterrows():
    seq_left = r["left_seq"]
    seq_right = r["right_seq"]

    det = deterministic_score_vector(seq_left, seq_right)
    pred_s, p_bind = predict_scores_and_bind(seq_left, seq_right)

    rows.append({
        "pdb": r["pdb"],
        "left_chain": r["left_chain"],
        "right_chain": r["right_chain"],
...
etype_panel = pd.DataFrame(rows)
etype_panel["tension_score"] = etype_panel["CONTACT_det"] - etype_panel["STRUCT_det"]
etype_panel.sort_values("tension_score", ascending=False).reset_index(drop=True)


# ==============================================================================

# Save script-visible table outputs that are displayed interactively.
if "ab_panel" in globals():
    ab_panel.sort_values("tension_score", ascending=False).reset_index(drop=True).to_csv(
        "antibody_antigen_decomposed_evidence.csv", index=False
    )
    print("Saved antibody_antigen_decomposed_evidence.csv")
if "etype_panel" in globals():
    etype_panel.sort_values("tension_score", ascending=False).reset_index(drop=True).to_csv(
        "enzyme_inhibitor_decomposed_evidence.csv", index=False
    )
    print("Saved enzyme_inhibitor_decomposed_evidence.csv")
